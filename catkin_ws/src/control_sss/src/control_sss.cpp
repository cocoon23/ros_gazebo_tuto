...
rosbag::Bag bag;
std::string save_folder="/mnt/hgfs/shared_folder/";
...
bool check_position(gazebo_msgs::ModelState ms)
{
	std::ifstream fstart_pos;
	std::string temp;
	std::vector<std::string> vstart_pos;
	fstart_pos.open("/mnt/hgfs/shared_folder/start_position.txt");
	if(fstart_pos.is_open())
	{
		while(!fstart_pos.eof())
		{
		...
		/*
		std::cout<<"test" <<(ms.pose.position.x == std::stof(vstart_pos[0]))<<(ms.pose.position.y == std::stof(vstart_pos[1])) << (ms.pose.position.z ==std::stof(vstart_pos[2])) << std::endl;
		if(ms.pose.position.x ==std::stof(vstart_pos[0])&&ms.pose.position.y ==std::stof(vstart_pos[1]))
			std::cout<<"hi"<<std::endl;	
		*/
		if(ms.pose.position.x == std::stof(vstart_pos[0]) && ms.pose.position.y == std::stof(vstart_pos[1]) && ms.pose.position.z == std::stof(vstart_pos[2]))
		{
			fstart_pos.close();
			return true;
		}
...
void save_bag(const sensor_msgs::Image& img_left, const sensor_msgs::Image& img_right, const sensor_msgs::PointCloud2& pt)
{
	bag.write("/sss/left/image_raw", ros::Time::now(), img_left);
        bag.write("/sss/right/image_raw", ros::Time::now(), img_right);
        bag.write("/sss/points2", ros::Time::now(), pt);
	ROS_INFO("bag");
...
*/
void save_point(const sensor_msgs::PointCloud2& msg_org, bool check_position)
{
...
int main(int argc, char **argv)
{
	...
	std::ifstream fstart_pos;
	fstart_pos.open("/mnt/hgfs/shared_folder/start_position.txt");
	ros::Publisher move_state = nh.advertise<gazebo_msgs::ModelState>("/gazebo/set_model_state", 1000);
	...
	while(ros::ok())
	{
		...
		while(!infile.eof())
		{
					...
					while(!fstart_pos.eof())
					{
						std::getline(fstart_pos, temp);
          					...
          					ms.pose.position.x = std::stof(vstart_pos[0]);
                				ms.pose.position.y = std::stof(vstart_pos[1]);
                				ms.pose.position.z = std::stof(vstart_pos[2]);
						std::cout<<"fstart_pos_eof"<<std::endl;
					}
					std::cout<<"model position: ["<< vstart_pos[0]<<","<< vstart_pos[1]<<","<< vstart_pos[2]<<"]" <<std::endl;

				}
			        move_state.publish(ms);
				ros::spinOnce();
				infile.close();
				infile.open("/mnt/hgfs/shared_folder/result.txt");
				line = 0;
			...
			if(i == line)
			{
				std::cout<<"angle: "<<angle<< " , "<<"line, i : " <<line<< ", "<<i<<std::endl;
				...
				split(temp, " ");ros::service::call("gazebo/set_model_configuration", mc);
				std::string current_folder = std::to_string(floorf(ms.pose.position.x*10)/10)+"_" + std::to_string(floorf(ms.pose.position.y*10)/10)+"_" +std::to_string(floorf(ms.pose.position.z*10)/10);
				if(mkdir((save_folder+current_folder).c_str(), 0776) == -1 && errno!=EEXIST)
				{
					fprintf(stderr, "%s directory create error: %s\n", strerror(errno));
					return -1;
				}
			        save_filename = vangle[0]+"_"+vangle[1]+".bag";
				save_imagefilename = vangle[0]+"_"+vangle[1];
				...
				boost::shared_ptr<sensor_msgs::Image const> sharedPtr_img_left, sharedPtr_img_right;
				...
				sharedPtr_pt = ros::topic::waitForMessage<sensor_msgs::PointCloud2>("/sss/points2");
				sharedPtr_img_left = ros::topic::waitForMessage<sensor_msgs::Image>("/sss/left/image_raw");
				sharedPtr_img_right = ros::topic::waitForMessage<sensor_msgs::Image>("/sss/right/image_raw");
				if(sharedPtr_pt == NULL)
					ROS_INFO("No point cloud messages recieved");
				else
					pt = *sharedPtr_pt;
                                if(sharedPtr_img_left == NULL)
                                        ROS_INFO("No sss/left/image_raw messages recieved");
                                else
				{
                                        left_img = *sharedPtr_img_left;
				        try
				        {
				        ...
				        cv::imwrite(save_folder+current_folder+"/"+save_imagefilename+"_left.jpg", cv_ptr->image);
				}
                                if(sharedPtr_img_right == NULL)
                                        ROS_INFO("No sss/right/image_raw messages recieved");
                                else
				{
					right_img = *sharedPtr_img_right;
				        try
				        {
					        cv_ptr = cv_bridge::toCvCopy(right_img, sensor_msgs::image_encodings::BGR8);
				        }
				        catch(cv_bridge::Exception& e)
				        {
					        ROS_ERROR("cv_bridge exception: %s", e.what());
				        }
				        cv::imwrite(save_folder+current_folder+"/"+save_imagefilename+"_right.jpg", cv_ptr->image);
				}

				bag.open(save_folder+current_folder+"/"+save_filename, rosbag::bagmode::Write);
				
				save_bag(left_img, right_img, pt);
				if(line == 0) 
				{
					save_point(pt,check_position(ms));
					std::cout<<"save_point"<<std::endl;
				}
				std::cout<<"save_bag"<<std::endl;
				//ros::spinOnce();
				//loop_rate.sleep();
				bag.close();
				line++;
		...
		std::string s_matoper;
		std::vector<std::string> v_matoper;
	        std::ifstream matlab;
                matlab.open("/mnt/hgfs/shared_folder/matlab_operate.txt");
		if(matlab.is_open())
		{
			std::getline(matlab, s_matoper);
			v_matoper = split(s_matoper, " "); 
		}
		else s_matoper ="0 1";

		std::cout << "s_matorper : " << s_matoper <<std::endl;
		matlab.close();
		if(stof(v_matoper[0]) == 1 && stof(v_matoper[1])==1)
		{
			std::cout << "matlab running!" << std::endl;
		}
		else if(stof(v_matoper[1]) == 0)
		{
			std::vector<std::string> s_speed;
			std::string temp_speed;
			std::ifstream sss_move;
			std::ofstream ofstart_pos;
			sss_move.open("/mnt/hgfs/shared_folder/speed.txt");
			std::getline(sss_move, temp_speed);
			s_speed = split(temp_speed, " ");
			ofstart_pos.open("/mnt/hgfs/shared_folder/start_position.txt", std::ios::app);
			if(std::abs(ms.pose.position.x)<3&& std::abs(ms.pose.position.y)<3 && std::abs(ms.pose.position.z)<5)
			ofstart_pos<<"\n"<<ms.pose.position.x + std::stof(s_speed[0])<<" "<<ms.pose.position.y + std::stof(s_speed[1])<<" "<<ms.pose.position.z + std::stof(s_speed[2]);
			sss_move.close();
			ofstart_pos.close();
			std::ofstream wmatlab;
                        wmatlab.open("/mnt/hgfs/shared_folder/matlab_operate.txt");
                        wmatlab<<"0 1";
                        wmatlab.close();
